...
void SPI_Init(void)
{
  ...
  P5SEL1 &= ~BIT7;
  P5SEL0 &= ~BIT7;
  P5OUT |= BIT7;             // Clear P1.0 output latch for a defined power-on state
  P5DIR |= BIT7;
  P2SEL0 &= ~(BIT0 | BIT1);
  P2SEL1 |= BIT0 | BIT1;
 ...
 * Description :
 *****************************************************************************/
unsigned char flash_memory_save_OTA_image(char *pkt)
{
...
void save_packet(char *pkt, unsigned int saving_idx,unsigned char data_type)
{
  ...
  debug_print("save data to flash");

  if(IS_SECTOR_ADDRESS(address) == TRUE)
  {
    debug_print("sector erase");
    W25Q16_Sector_Erase(address);
  }

  debug_print("write spi");
  W25Q16_Write(address, local_storage_buff, W25Q16_PAGE_SIZE);
  extFlashPowerDown();
  debug_print("write spi done");

  memset(local_storage_buff, 0, W25Q16_PAGE_SIZE);
  switch(data_type)
  {
    case 1:   
      flash_memory.put_temp_idx++;
