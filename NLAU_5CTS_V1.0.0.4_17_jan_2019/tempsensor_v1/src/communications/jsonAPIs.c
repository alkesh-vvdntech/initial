...
void json_print_temp_obj(json_printer *printer, int sensor, time_t timeUTC)
{
    ...
    if ((sensorState->state.incursion) && (!(sensorState->state.excursion)))	// Excursion
    {
    ...
    else if((sensorState->state.excursion) && (!(sensorState->state.incursion)))	// Incrusion
    {
	...
	sensorState->state.excursion = false;
	sensorState->state.incursion = false;
	g_pSysState->system.switches.tempBoundaryCrossed = false;
    }
    json_print_raw(printer,JSON_OBJECT_BEGIN, NULL, 0);
...
void json_print_simDevRdy_obj(json_printer *printer, int slot)
{
    ...
    slot = 0; //the only allowed non-zero value is 1, ***dual-sim hardware
    simId = g_pDevCfg->SIM[0].cfgSimId;
 //   simPhoneNum =  g_pDevCfg->SIM[0].cfgPhoneNum;
    simPhoneNum = NULL;
...
void json_print_devDevRdy_obj(json_printer *printer)
{
    ...
    json_print_raw(printer,JSON_STRING, "CT5Wifi", 7);
