...
void release_buf()	// Telit - Modification not required
{
...
int8_t data_send_api_packet(API_PACKET_TYPE apiType)	// Telit but need to make compitable with SIM800
{
    ...
    strcat(http_url,DEVICE_ALARM_URL_PATH);
    //res = http_send_file(filr, (char *)apiInfo->apiURL);
    res = http_send_file(filr, http_url);
    log_enable();
...
int8_t data_send_alarm()	// For telit but need to update for sim800
{
    int uart_state;
    uint16_t lineSize = 0;
...
//    }
    ...
    http_open_connection_upload(strlen(line), g_ConfigDevice.cfgDeviceAlarm_URL);
...
int8_t data_send_http(FIL *file, uint32_t start, uint32_t end)
{
    ...
    debug_print("Check Now");
    while(1);

    length = strlen(line) + (end - file->fptr) + strlen(footer);

//    uart_tx_nowait(line);	    // Send the date line
...
//		uart_setOKMode();
		http_open_connection_upload(length, g_pDevCfg->cfgUpload_URL);
