...
void alarm_sms(uint8_t Id, time_t elapsed)
{
	...
	//if (sensor->fTemperature >= g_pDevCfg->stTempAlertParams[Id].threshHot)
      if (sensor->fTemperature >= 40)
	{
            //buzzer_start();
	    lcd_printl(LINEC,"hot temp breach");
	...
	//if (sensor->fTemperature <= g_pDevCfg->stTempAlertParams[Id].threshCold)
      if (sensor->fTemperature <= 25)
	{
            //buzzer_start();
            lcd_printl(LINEC,"cold temp breach");
   ...
   ///memcpy(g_pDevCfg->cfgSMSNumbers[0].cfgReportSMS, "+917503594853", 13);
   //memcpy(g_pDevCfg->cfgSMSNumbers[1].cfgReportSMS, "+918650347439", 13);
