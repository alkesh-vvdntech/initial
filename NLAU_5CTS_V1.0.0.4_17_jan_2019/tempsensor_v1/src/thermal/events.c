...
//void event_display_off(time_t currentTime) {
    lcd_turn_off();
}
...
void event_subsample_temperature(void *event, time_t currentTime)
{
  ...
  UINT bytes_written = 0;
  //disable_diagnosis();
  lcd_printl(LINEC,"SAMPLING TEMP");
...
//void event_upload_samples(time_t currentTime)
{
...
void upload_samples(void)
{
    ...
    //fat_init_drive();    
    debug_print("\r\nUpload EVENT\r\n");
    lcd_printl(LINEC,"Uploading data");    //LCD PRINT
    disable_diagnosis();      //ap
    ...
    memset(upload_buffer,'\0',sizeof(upload_buffer));
    sprintf(upload_buffer,"{\"vId\":\"Nexleaf\",\"data\":[{\"dId\":\"%s\",\"tmps\":[",device_imei);  //device_imei,g_pDevCfg->cfgIMEI
...
//void event_reboot_system(time_t currentTime) {
#ifndef _FISM_
    log_reboot("EVENT");
