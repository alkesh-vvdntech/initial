...
extern unsigned char fw_version[10]; 
...
uint8_t lcd_show()
{
  ...
  if (iItemId == g_iDisplayId && lastRefresh == rtc_get_second_tick())
  {
    return 0;
  }
  ...
  lcd_printl(LINEC,lcdBuffer);
  if(!flags.sim_ready)
  {
    lcd_clear();
    lcd_printl(LINEC,"SIM ERR");
    return 0;
  }
  ...
  if(flags.lcd_update)
  {
    switch(flags.button_int_val)
    {
      case 1:
        strcat(lcdBuffer,"Sens C: ");
        if(temperature_getString(0)!='\0')
        {            
        ...
        lcd_printl(LINE2,lcdBuffer);
        memset(lcdBuffer,'\0', sizeof(lcdBuffer));
        break;
    
      case 2:
        strcat(lcdBuffer,"Sens D: ");
        if(temperature_getString(1)!='\0')
        {
      ...
      case 3:
        ...
        strcat(lcdBuffer, "BATT: %  ");
        lcd_printl(LINE2,lcdBuffer);
        memset(lcdBuffer,'\0', sizeof(lcdBuffer));
        break;

    case 4: 
        strcat(lcdBuffer,device_imei);
        lcd_printl(LINEC,"    IMEI    ");
        lcd_printl(LINE2,lcdBuffer);
        memset(lcdBuffer,'\0', sizeof(lcdBuffer));
        break;

    case 5: 
        strcat(lcdBuffer,fw_version);
        lcd_printl(LINEC,"    FW VERSION    ");
        lcd_printl(LINE2,lcdBuffer);
        flags.button_int_val = 0;
        memset(lcdBuffer,'\0', sizeof(lcdBuffer));
        break;
    ...
    default:   
        memset(lcdBuffer,'\0', sizeof(lcdBuffer));
        strcat(lcdBuffer,"SS-");
        strcat(lcdBuffer,itoa_pad(dvalue.signal_st));
        lcd_printl(LINE2,lcdBuffer);
        memset(lcdBuffer,'\0', sizeof(lcdBuffer));
        break;
    }
...
//sachin         lcd_setCustomDate(lcdBuffer, errSlot->timeStamp);
	...
	//display the lines
        lcd_printl(LINEC,(uint8_t *) lcdBuffer);
        lcd_printl(LINE2,(uint8_t *) &lcdBuffer[iIdx]);
...
void lcd_setupIO() {
	PJDIR |= BIT6;      			// set LCD reset and Backlight enable
        PJDIR |= BIT7;
