...
#define halSPIRXBUF  UCA1RXBUF
 ...
 #define halSPIRXBUF  UCB2RXBUF
 #define halSPI_SEND(x) UCB2TXBUF=x
 #define halSPITXREADY  (UCB2IFG & UCTXIFG)         /* Wait for TX to be ready */
 #define halSPITXDONE  (UCB2STATW & UCBUSY)       /* Wait for TX to finish */
 #define halSPIRXREADY (UCB2IFG & UCRXIFG)          /* Wait for TX to be ready */
 #define halSPIRXFG_CLR UCB2IFG &= ~UCRXIFG
...
//put this in FRAM
    ...
    #pragma SET_DATA_SECTION(".sd_raw_block")
    /* static data buffer for acceleration */
    static uint8_t raw_block[512];
    #pragma SET_DATA_SECTION()
  #elif defined(__IAR_SYSTEMS_ICC__)
    /* static data buffer for acceleration */
    #pragma location = "RAW_BLOCK"
    __no_init static uint8_t raw_block[512];
  #else
...
static uint8_t sd_raw_send_byte(uint8_t b);
...
uint8_t sd_raw_init()
{
    ...
    */
    ...
    unselect_card();
    ////delay(2);     shubham change;    shubham change
	...
	// MSB
	UCB2CTLW0 |= UCMST | UCSYNC | UCCKPL | UCMSB; //Mode3 3-pin, 8-bit SPI master, clock polarity=1, clock phase=0 (changed on first edge and captured in next)
//	UCA1CTLW0 |= UCMST | UCSYNC | UCMSB;
	// MSB
	UCB2CTLW0 |= UCSSEL__SMCLK;                // SMCLK - 8MHz
	//UCA1BR0 = 0x00;                           //  SPI clk - 8MHz
	//UCA1BR1 = 0;
	UCB2BR0 = 8; //400khz
	UCB2BR1 = 0x0;
	//UCA1BRW = 20;                           //  SPI clk - 400KHz
	//UCB2MCTLW = 0;                            // No modulation
         UCB2IE |= UCRXIE;
 ...
 * \see sd_raw_rec_byte
 */
uint8_t sd_raw_send_byte(uint8_t b)
{
	//unsigned int rbuff[2];
	/* atmel code
    SPDR = b;
    ...
    while(length > 0)
    {
...
#endif
        {
            ...
            //for(k=0;k<10;k++)
            	//for(j=0;j<100;j++);
            //delay(30);
            response = sd_raw_send_command(CMD_READ_SINGLE_BLOCK, (sd_raw_card_type & (1 << SD_RAW_SPEC_SDHC) ? block_address / 512 : block_address));
...
//delay(2);     shubham change;
        ...
        uint8_t response;
        //delay(30);
        response = sd_raw_send_command(CMD_WRITE_SINGLE_BLOCK, (sd_raw_card_type & (1 << SD_RAW_SPEC_SDHC) ? block_address / 512 : block_address));
