...
int8_t modem_getOwnNumber() {
	...
	if(sim->cfgPhoneNum[0] == ',') memset(sim->cfgPhoneNum, 0, sizeof(sim->cfgPhoneNum));
...
int8_t modem_get_Sim_Id() {
        ...
        modem_ignore_next_errors(1); // Ignore 1 error since we know our sim cards dont support this command
        state = modem_parse_string("AT+CCID?", "+CCID: ", sim->cfgSimId,
        SID_MAX_LEN + 1);
        modem_ignore_next_errors(0);
...
//sachin	uart_tx_timeout("AT",5000,10); // Display OK
        //uart_tx("AT+CSMP?");
        // We are not doing master reset anymore because if power goes off during master reset it could cause problem.
  /*
        if (g_bServiceMode == true) {
          uart_tx_timeout("AT+CFUN=4",5000,2);
          uart_tx_timeout("AT+CMAR=00000000\r\n",10000,2);
          delay(5000);
        }
    */
	...
	//uart_tx("#BNDLOCK=1");  //bandlock on 900MHz
	//uart_tx("#BNDLOCK=2"); //bandlock on 1800MHz
	...
	uart_tx("AT+CCED=0");
        if (g_pSysCfg->numberConfigurationRuns < 3 && g_pSysCfg->modemFirstInit
            || state_SimIdChanged()) {
          config_get_apn(uart_getRXHead(),g_pDevCfg->cfgSelectedSIM_slot); // get APN based on MCC & MNC
        }
...
//sachin	modem_getSMSCenter();
//sachin	modem_getSignal();
	...
	uart_tx("+CSDH=1"); // Show Text Mode Parameters - +CSDH  Set command controls whether detailed header information is shown in text mode (+CMGF=1) result codes
        ...
        uart_tx_timeout("AT+CSMP=17,11,0,0\r\n",10000,2);  // After Master reset data coding scheme of SMS changes for 4G SIM.(Fourth Argument) This is to fix it.
 //       uart_tx_timeout("AT+CSMP?",10000,2);   // To read the data coding scheme of 2G/4G SIM
...
//	http_enable(); //Required for AT#NTP inside modem_pull_time
	modem_pull_time();
...
char get_sim_num()
{
      ...
      PARSE_NEXTSTRING(token, sim_num, 20, "\"", UART_FAILED);
      ...
      memcpy(g_pDevCfg->SIM[0].cfgPhoneNum, sim_num, 20);
      return 1;
}
/**********************************************************************************************
 ...
 **********************************************************************************************/
unsigned char send_data(char *packet)
{
  char http_url[150];
  ...
  if((flags.sim_ready) &&(flags.internet_flag))
  {
    ...
    //sprintf(http_url,"AT+HTTPPARA=\"URL\",\"http://%s/coldtrace/uploads/l/v4/\"\r\n",flash_memory.server_url);
     sprintf(http_url,"AT+HTTPPARA=\"URL\",\"http://dev.coldtrace.org/uploads/fsdk/v1/\"\r\n");
...
{       
  ...
  if(ret)
  {
     ...
     snprintf(ccid,21,ret-24);        // changed the value of ccid 
  }
}
/**********************************************************************************************
...
void check_gsm_fw()
{
  ...
  if(ret)
  {
    memset(gsm_fw,'\0',sizeof(gsm_fw));
    snprintf(gsm_fw,17,ret-20);
  }
}
/**********************************************************************************************
