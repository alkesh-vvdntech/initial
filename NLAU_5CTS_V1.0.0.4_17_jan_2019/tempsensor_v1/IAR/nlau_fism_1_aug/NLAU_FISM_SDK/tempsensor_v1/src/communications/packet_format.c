...
void append_data_in_buffer()
{
  ...
  for(sensor_count = 0;sensor_count<3;sensor_count++)
  {
    if(temp_disconnt[sensor_count]== '0')
    {
      ...
      sprintf(temp_data,"{\"sId\":\"%c\",\"time\":%ld,\"tmp\":%s}",sensor[sensor_count],rtc_getUTCsecs(),dvalue.sensor[sensor_count].value);
  ...
  strcat(sampled_data,temp_data);
  sampled_data[strlen(sampled_data)] = ',';
  sprintf(temp_data,"{\"sId\":\"fridge\",\"batt\":%d,\"pwr\":%d,\"ss\":%d,\"time\":%ld}",batt_percent,flags.batt_connected,dvalue.signal_st,rtc_getUTCsecs());
