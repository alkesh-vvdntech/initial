...
#ifdef _FISM_
void i2c_temperature(uint8_t iSensorIdx)
{
    float tempfloat = 0.0;
    ...
    i2c_read(slaveadr,0xE3, 2, temper);
    temp = (((temper[0]<<8) & 0xFFFF) | (temper[1] & 0xFF));
    temp_data = ((175.72 * temp)/65536) - 46.85;
    memset(dvalue.sensor[2].value,'\0',sizeof(dvalue.sensor[2].value));
    getFloatNumber2Text(temp_data,dvalue.sensor[2].value);

#ifdef _FDEBUG_
        debug_print("\n\rsensor C = ");
        debug_print(dvalue.sensor[2].value);
        debug_print("\n\r");
#endif
    if ((float)A0tempdegC < 0.0) {
      tempfloat = ((float)-0.0062 * temp_data)  + (float)0.1493;
      temp_data = (float)temp_data - (float)tempfloat;
    }

    if ((float)temp_data > TEMP_CUTOFF) {
    if(cfgGetAppMode() == APP_MODE_ST) {
        sensorState->state.disconnected = false;
    }
    if((float)sensor->fTemperature != TEMP_BOOT_MAGIC_VAL) {
        if((float)temp_data >= (float)hot && (float)sensor->fTemperature < (float)sensor->lastThreshHot) {		//Hot Excursion
            sensorState->state.excursion = true;
        }
        else if((float)temp_data <= (float)cold && (float)sensor->fTemperature > (float)sensor->lastThreshCold) {	//Cold Excursion
            sensorState->state.excursion = true;
        }
        else if((float)temp_data < (float)hot && (float)sensor->fTemperature >= (float)sensor->lastThreshHot) {	//Hot Incursion
            sensorState->state.incursion = true;
        }
        else if((float)temp_data > (float)cold && (float)sensor->fTemperature <= (float)sensor->lastThreshCold) {	//Cold Incursion
                        sensorState->state.incursion = true;
        }
        else {
        ...
        if(temp_data >= hot || temp_data <= cold) {		//Hot Excursion
            sensorState->state.excursion = true;
        } else {
            sensorState->state.incursion = true;
        }
...
float resistance_to_temperature(float R) {
	...
	// Thermistor USSensor KS103J2
        //float A1 = 0.003353704, B1 = 0.000256595, C1 = 0.000002561571, D1 = 0.00000004108811,
        //                 tempdegC;
	//float R25 = 10000.0;
    float tempdegC = 0;
...
void temperature_subsampling_calculate(int8_t iSensorIdx) {
	...
	if ((float)A0tempdegC > TEMP_CUTOFF) {
        ...
        if((float)sensor->fTemperature != TEMP_BOOT_MAGIC_VAL) {
            ...
            if(A0tempdegC >= hot || A0tempdegC <= cold) {		//Hot Excursion
                sensorState->state.excursion = true;
            } else {
                sensorState->state.incursion = true;
            }
...
void temperature_analog_to_digital_conversion() 
{
	...
	if (tem->iSamplesRequired == 0)
		return;
          ...
          for (iIdx = 0; iIdx < SYSTEM_NUM_SENSORS-1; iIdx++)  
          {
